  ...
  <div class="page-body">
    <div class="container-fluid">
      <div class="page-title">
        <div class="row">
          <div class="col-sm-6 col-12"><h2>Kelola Penerbit</h2></div>
            ...
            <div class="card-header card-no-border pb-0">
              <div class="header-top">
                ...
                <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#modalTambah">
                  <i class="iconly-Plus icli me-1"></i> Tambah Penerbit
                </button>
              </div>
            </div>
                ...
                <table class="table display table-bordernone" style="width:100%">
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td>{{ $p->kota ?? '-' }}</td>
                      <td>{{ $p->telepon ?? '-' }}</td>
                      <td><span class="badge bg-light-primary text-primary">{{ $p->buku->count() }} buku</span></td>
                    ...
                    @empty
                    <tr><td colspan="6" class="text-center text-muted py-4">Belum ada penerbit</td></tr>
                    @endforelse
                  </tbody>
                </table>
              </div>
    ...
    <div class="modal-content">
      <form action="{{ route('admin.penerbit.store') }}" method="POST">
        ...
        <div class="modal-body">
          <div class="mb-3">
            <label class="form-label fw-semibold">Nama Penerbit <span class="text-danger">*</span></label>
            <input type="text" name="nama" class="form-control" placeholder="Nama penerbit" required>
          </div>
          <div class="mb-3">
            <label class="form-label fw-semibold">Kota</label>
            <input type="text" name="kota" class="form-control" placeholder="Kota penerbit">
          </div>
          <div class="mb-3">
    ...
    <div class="modal-content">
      <form id="formHapus" method="POST">
        ...
        <div class="modal-header border-0 pb-0">
          <h5 class="modal-title text-danger">Hapus Penerbit</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
        </div>
        <div class="modal-body pt-2">
          <p class="mb-0">Yakin hapus penerbit <strong id="hapusNama"></strong>?</p>
...
<script>
  document.querySelectorAll('.btn-edit').forEach(btn => {
    btn.addEventListener('click', function() {
      document.getElementById('formEdit').action = `/admin/penerbit/${this.dataset.id}`;
  ...
  document.querySelectorAll('.btn-hapus').forEach(btn => {
    btn.addEventListener('click', function() {
      document.getElementById('formHapus').action = `/admin/penerbit/${this.dataset.id}`;
